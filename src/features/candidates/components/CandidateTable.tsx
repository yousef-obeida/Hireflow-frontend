      ...
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="bg-white/50">
          {candidates.map((candidate) => {
            ...
            // Generate pseudo AI match for design purposes or use real one when available
            const nameLen = candidate.full_name?.length || 10;
            const aiScore = 50 + (nameLen * 3) % 49; // 50-98 range
            ...
            return (
              ...
              >
                ...
                <td className="px-6 py-4 text-sm text-[#111c2d] whitespace-nowrap">
                  {role}
                </td>
                <td className="px-6 py-4 text-center">
                  <span className={`inline-flex items-center justify-center rounded-3xl px-2.5 py-1 text-xs font-semibold ${
                    aiScore >= 90 ? 'text-green-500' : 
